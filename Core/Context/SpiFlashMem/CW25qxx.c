...
void WindbondInit()
{
//	HAL_GPIO_WritePin(SPI_HOLD_GPIO_Port, SPI_HOLD_Pin, GPIO_PIN_SET);
//	HAL_GPIO_WritePin(SPI_WP_GPIO_Port, SPI_WP_Pin, GPIO_PIN_SET);
}
void WinbondTest1()
{
	WindbondInit();
	W25qxx_Init();
	uint32_t flashid = W25qxx_ReadID();
	asm("nop");
	uint8_t fileID[16];
	uint8_t wrData[16];
	...
	W25qxx_ReadBytes(fileID, 0, 16);

	asm("NOP");


	if (fileID[0] == 'S' && fileID[1] == 'H')
	{
		asm("NOP");
	}
	else
	{
		...
		W25qxx_WriteBlock(wrData, 0, 0, 16);
	}
	asm("NOP");
