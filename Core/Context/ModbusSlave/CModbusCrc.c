#include "CModbusCrc.h"

/*------------------------- MODBUS CRC16 -----------------------------*/
const uint8_t gCrc_high_table[] =
{  };


const uint8_t gCrc_low_table[] =
{  };

uint8_t CRC16Calculate(MODBUS_DATA_t *pMb_t, uint8_t pBytes)
{
	uint8_t tIndex = 0, m;
	pMb_t->mCRCHi = 0xFF;
	pMb_t->mCRCLo = 0xFF;
	for (m = 0; m < pBytes; m++)
	{
		tIndex = pMb_t->mCRCLo ^ pMb_t->mModbusBuffer[m];
		pMb_t->mCRCLo = pMb_t->mCRCHi ^ gCrc_high_table[tIndex];
		pMb_t->mCRCHi = gCrc_low_table[tIndex];
	}
	return (pMb_t->mCRCHi | pMb_t->mCRCLo);
}
