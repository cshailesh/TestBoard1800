...
uint8_t CRC16Calculate(ModbusBase_t *m_t, uint8_t Byte_number)
{
	...
	m_t->CRCHi = 0xFF;
	m_t->CRCLo = 0xFF;
	for (m = 0; m < Byte_number; m++)
	{
		tIndex = m_t->CRCLo ^ m_t->ModbusBuffer[m];
		m_t->CRCLo = m_t->CRCHi ^ Crc_high_table[tIndex];
		m_t->CRCHi = Crc_low_table[tIndex];
	}
	return (m_t->CRCHi | m_t->CRCLo);
}
